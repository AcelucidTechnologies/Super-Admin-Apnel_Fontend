...
export class EditProfileComponent {
  ...
  getLeaveById() {
    this.leaveservice.getLeaveProfileById(this.id).subscribe((res) => {
      ...
      const modifiedTime = this.convertDateFormat(res.systemFields.modifiedTime);
      ...
      this.profileForm.patchValue({
        ...
        personalDetails: {
          ...
          identityInformation: {
            uan: res?.identityInformation?.uan ?  res?.identityInformation?.uan : '',
            panNumber: res?.identityInformation?.panNumber ?  res?.identityInformation?.panNumber: '',
            aadharNumber: res?.identityInformation?.aadharNumber? res?.identityInformation?.aadharNumber : '',
        },
      ...
      res.workExperience.map((item)=>{
        ...
        res.educationDetails.map((item)=>{
          this.educationDetails.push(this.fb.group({
            ...
            toDate: this.convertDateFormat(item.toDate),
            dateOfCompletion: this.convertDateFormat(item.dateOfCompletion),
...
editUserProfile() {
  this.leaveservice.editProfile(this.payload, this.id).subscribe(
    (res) => {
      if (res) {
        this.toastr.showSuccess('user profile edit successfully', 'profile edit');
        this.ngxLoader.stop();
